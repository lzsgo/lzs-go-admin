...
### 1.2 贡献指南
Hi! 首先感谢你使用 lzs-go-admin。

lzs-go-admin 是一套为快速研发准备的一整套前后端分离架构式的开源框架，旨在快速搭建中小型项目。

lzs-go-admin 的成长离不开大家的支持，如果你愿意为 lzs-go-admin 贡献代码或提供建议，请阅读以下内容。
...
使用 `Goland` 等编辑工具，打开server目录，不可以打开 lzs-go-admin 根目录
...
path: 'xdg-open',
执行以下命令重新启动
touch /usr/bin/xdg-open && chmod +x /usr/bin/xdg-open

修改.env.development
VITE_BASE_PATH=本机地址 而不是127.0.0.1
```
...
使用`VSCode`打开根目录下的工作区文件`lzs-go-admin.code-workspace`，在边栏可以看到三个虚拟目录：`backend`、`frontend`、`root`。
...
- restful示例：可以参考用户管理模块中的示例API。
	- 前端文件参考: [web/src/view/superAdmin/api/api.vue](https://github.com/lzsgo/lzs-go-admin/blob/master/web/src/view/superAdmin/api/api.vue)
    - 后台文件参考: [server/router/sys_api.go](https://github.com/lzsgo/lzs-go-admin/blob/master/server/router/sys_api.go)
